...
// in this test let's using LeaderboardScoreAsc.
const Leaderboard = require('../index').LeaderboardScoreAsc;

describe('LeaderboardScoreAsc', function () {
    let redis, sandbox;
    ...
    describe('test scenario running.', function () {
        ...
        before(function () {
            return Leaderboard.create(redis, "lbTest", true)
            .then((_lb) => {
                lb = _lb;
            })
        });
            ...
            it('make sure it works as we designed.', function () {
                ...
                .then(() => {
                    ...
                    .then((res) => {
                        ...
                        assert.deepEqual(res, { name: 'Michael', score: 300, rank: 5 });
                    });
                })
                .then(() => {
                    // Adam is not in yet.
        ...
        describe('getNeighbors.', function () {
            it('make sure it works as we designed.', function () {
                ...
                .then((res) => {
                    ...
                    assert.deepEqual(res, {
                        list: [
                            { name: 'John', score: 100, rank: 1 },
                            { name: 'Eric', score: 150, rank: 2 },
                            { name: 'Scott', score: 200, rank: 3 },
                            { name: 'Bryan', score: 250, rank: 4 },
                            { name: 'Michael', score: 300, rank: 5 }
                        ]
                    });
                });
            });
            ...
            it('make sure it works as we designed. case clipped.', function () {
                ...
                return lb.getNeighbors("John", 1)
                .then((res) => {
                    assert.strictEqual(spyGetPosition.callCount, 1);
                    assert.strictEqual(spyGetPosition.args[0].length, 1);
                    assert.strictEqual(spyGetPosition.args[0][0], "John");
